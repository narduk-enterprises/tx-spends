...
The goal is a **single coherent pipeline**: Tailwind v4 + **@nuxt/ui** + Nuxt 4.
Follow the **nuxt-ui** skill (`references/theming.md`, `references/installation.md`)
and **nuxt** skill (`references/nuxt-config.md`) so you do not bypass or break the
module’s semantic system.

### 2.0 Nuxt config & app shell (verify first)

**`nuxt.config.ts` (or app equivalent)**

- Include **`@nuxt/ui`** in `modules`.
- Register the stylesheet that chains Tailwind + Nuxt UI + brand, e.g.
  `css: ['~/assets/css/main.css']` (path must match the app).

**`app.vue` (or root layout)**

- Wrap the page tree in **`<UApp>`** so Nuxt UI overlays, tooltips, and color
  semantics behave correctly.

**Checklist**

- [ ] `@nuxt/ui` is in `modules`; no second conflicting Tailwind entry that skips `@nuxt/ui`.
- [ ] One CSS entry loads `tailwindcss` → `@nuxt/ui` → `./brand.css` (see 2b).
- [ ] `<UApp>` wraps `<NuxtPage />` (or equivalent).
- [ ] `colorMode` in `nuxt.config` matches how the app toggles light/dark (Nuxt Color Mode + Nuxt UI).

### 2a. Set Nuxt UI colors (`app.config.ts`)

Create or update `apps/web/app/app.config.ts` with **`defineAppConfig`**. Map
`primary` and `neutral` to **Tailwind palette names** Nuxt UI supports (e.g.
`sky`, `zinc`). Add **`secondary`**, **`success`**, **`error`**, etc. when it
keeps the product coherent — see nuxt-ui semantic color table.
...
export default defineAppConfig({
  ui: {
    colors: {
      ...
      // Optional: align with brand / UI semantics
      // secondary: '...',
      // success: 'emerald',
      // error: 'rose',
    },
...
**Custom color names** (not from Tailwind’s built-in scales): only if the
product needs them — then follow nuxt-ui theming: register in `nuxt.config`
`ui.theme.colors` and define **all** shade variables (`--color-<name>-50` …
`950`) in `@theme`. Prefer named Tailwind colors for speed and consistency.

### 2b. Set Tailwind v4 theme overrides (`brand.css`)

Create or update `apps/web/app/assets/css/brand.css` and import it **after**
`@nuxt/ui` in `main.css` so fonts, radii, and shadows extend Nuxt UI’s theme
without replacing its color system.

> _Note: For standard UI surfaces, prefer **Nuxt UI semantic classes**
> (`text-primary`, `text-muted`, `bg-default`, `bg-elevated`, `border-default`)
> and `app.config.ts` color mappings — not one-off hex on buttons and cards.
> Use `var(--color-primary-500)` only where it stays consistent with the mapped
> primary._
...
Import order in `apps/web/app/assets/css/main.css` (**do not reorder** — Nuxt UI
must load after `tailwindcss` and before brand extensions):
...
Create **bespoke, high-fidelity imagery** — not clipart, not stock, not
placeholder.

**Image tool order (mandatory):**

1. **Use built-in / host tools first** — the agent or IDE’s native image
   capability (e.g. `GenerateImage`, `generate_image`, or equivalent) when
   present. Prefer this path so assets are created with the least setup.
2. **Fallback: Narduk CLI** — if no built-in tool is available, it errors, or
   you need local xAI-backed batch output: use the **narduk-tool** (`/narduk-tool`)
   skill: `narduk-cli image generate` (or `narduk image generate`), `--aspect-ratio`
   for 16:9 heroes, `--output-dir` to land files under the app. Copy or convert
   outputs into the paths below. Credentials: see that skill (`XAI_API_KEY` / Doppler).

Apply the same prompt formulas and quality bars regardless of which tool ran.
...
### 3d. Resize, crop, and encode (local tools — mandatory)

Generators often return the wrong resolution or aspect. **Always** post-process
with **locally available** CLIs before final paths are considered done. Pick
whatever is installed; do not assume a specific global install order:

| Need                   | Examples (use what exists)                                                                                            |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Crop to aspect + scale | ImageMagick `magick input.png -resize 1920x1080^ -gravity center -extent 1920x1080 out.webp` (adjust for your format) |
| WebP encode / resize   | `cwebp`, `ffmpeg -i ... -vf scale=...`, or Node `sharp`                                                               |
| macOS quick resize     | `sips -z height width file` (maintain aspect carefully)                                                               |
| Batch / scripted       | Small `node -e` using `sharp` if the workspace already depends on it                                                  |

**Targets (tune to product, but be explicit):**

- **Hero / atmospheric** (`hero-bg.webp`): true **16:9**, e.g. **1920×1080** (or
  2560×1440 for retina); center-crop if the source is not 16:9; compress WebP
  sensibly.
- **Empty states** (`empty-*.webp`): **fixed square** canvas (e.g. 512×512 or
  1024×1024) so layout does not jump; letterbox or crop consistently.
- **Logo source**: prefer **SVG** in `favicon.svg`; if the master is raster,
  provide a **large square** source (e.g. 1024×1024) before Phase 4 — the
  favicon script derives 16/32/180 from it, but garbage-in hurts small sizes.
