...
1. **Check deploy scripts:** // turbo
   ...
   - Expected pattern: `"deploy": "wrangler deploy"`. `pnpm ship` runs `pnpm build`
     in the app dir before `pnpm deploy`, so `deploy` must not re-run `nuxt build`.
   - For a one-off local deploy without ship, run `pnpm build` then `pnpm deploy`
     (with Doppler as usual).
